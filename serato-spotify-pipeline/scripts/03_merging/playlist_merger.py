...
class PlaylistMerger:
    def __init__(self, master_file: str, playlists_directory: str, taxonomy_file: str):
        ...
        # Initialize enrichment columns if they don't exist
        for column in ['vibe', 'sound', 'culture', 'emotional_layers', 'prominent_instruments', 'placement']:
            if column not in self.master_df.columns:
                self.master_df[column] = ""
                logger.info(f"Created new column: {column}")
    ...
    def load_playlists(self) -> dict:
        ...
        for csv_file in csv_files:
            ...
            # Skip ignored files
            if playlist_name in ignored_files:
